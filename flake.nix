{
  description = "__TITLE__ (J2ME) decompilation, resource analysis, and native Rust port environment";
    ...
    in
    {
      apps = forAllSystems (
        ...
        let
          ...
          #   __RESOURCES_ENV__=<same> nix run .#fetch-resources
          fetchResources = {
            type = "app";
            program = toString (
              pkgs.writeShellScript "fetch-resources" ''
                ...
                source="''${1:-''${__RESOURCES_ENV__:-}}"
                if [ -z "$source" ]; then
                  echo "usage: nix run .#fetch-resources -- <checkout-path-or-git-url>" >&2
                  echo "   or: __RESOURCES_ENV__=<same> nix run .#fetch-resources" >&2
                  exit 1
                fi
                if [ -d "$source" ]; then
                  resources=$source
      ...
      devShells = forAllSystems (
        ...
        in
        {
          default = pkgs.mkShell {
            ...
            RUST_BACKTRACE = "1";
            shellHook = ''
              echo "__TITLE__ — J2ME recovery environment"
