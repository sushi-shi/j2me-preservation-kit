...
| Per-node AST crosswalk | `just crosswalk-check` (+ `python3 -m unittest tools.tests.test_crosswalk_validator`) | `just crosswalk-canfail` (`--self-test`: dropped decision, coarse blanket, and bytecode/Java-AST/Rust-AST digest perturbations each go red, a one-node evidence drift breaks the node lock, a substituted Rust literal trips index parity, and removing a reasoned interleave owner group exposes A-B-A-B crossing ownership). `just crosswalk-fixture-canfail` proves the coarse-blanket, div-vs-`sm()`, `entity_row[13]`-vs-`[10]`, and unjustified temporal-interleave bug rows go red. |
