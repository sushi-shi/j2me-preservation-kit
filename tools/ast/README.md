...
4. **Literal / index parity.** Inside a paired `op`, the numeric constants each
   ...
   arguments — must be equal in value (hex/decimal/typed forms normalized, so Rust
   `0xff` matches Java `255`). A Rust `entity_row(..)[13]` paired against the
...
5. **Hash locks.** The bytecode Code-attribute digest, the full javac AST digest,
...
`fixtures/` holds two self-contained reproductions, regenerated by
...
the literal/index parity tooth.
